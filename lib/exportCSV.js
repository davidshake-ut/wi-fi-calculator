  ...
  for (const { title, bom } of sections) {
    ...
    if (hasServices) {
      ...
      for (const i of bom.serviceItems) {
        rows.push(['Services', i.sku, i.description, 1, i.unitPrice.toFixed(2), i.totalPrice.toFixed(2)]);
      }
    }
    ...
    if (hasServices) {
      rows.push(['', '', `${title} Professional Services`, '', '', bom.totalServicesPrice.toFixed(2)]);
    }
    rows.push(['', '', `${title} Shipping (7%)`, '', '', bom.shippingPrice.toFixed(2)]);
