...
export function calculateCameraBOM(cameraInputs = {}, priceOverrides = {}, allProducts = []) {
  const items = [];
  ...
  // --- Financial totals (hardware only; 7% shipping; no services) ---
  const totalHardwareCost = items.reduce((s, i) => s + i.totalCost, 0);
  const totalHardwarePrice = items.reduce((s, i) => s + i.totalPrice, 0);
  const totalServicesCost = 0;
  const totalServicesPrice = 0;
  const shippingCost = totalHardwareCost * 0.07;
  const shippingPrice = totalHardwarePrice * 0.07;
  const grandTotalCost = totalHardwareCost + shippingCost;
  const grandTotalPrice = totalHardwarePrice + shippingPrice;
  const overallMargin =
