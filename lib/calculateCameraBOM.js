...
// (serviceItems is empty — camera systems carry no professional services yet).
...
export function calculateCameraBOM(
  ...
  // --- Professional services (camera labor, separate from the Wi-Fi labor) ---
  ...
  if (totalCameras > 0) {
    const installHours = Math.ceil(totalCameras * 0.75); // ~45 min per camera
    addService(
      'CAM-INSTALL',
      `Camera Installation & Mounting (est. ${installHours} hrs @ $100/hr)`,
      installHours * 75,
      installHours * 100
    );

    const configHours = Math.max(2, nvrCount * 2); // ~2 hrs per NVR
    addService(
      'CAM-CONFIG',
      `NVR & Camera Configuration (${nvrCount} NVR${nvrCount === 1 ? '' : 's'})`,
      configHours * 75,
      configHours * 100
    );
    ...
    addService(
      'CAM-TEST',
      `Camera Testing & Validation (${totalCameras} cameras)`,
      Math.ceil(totalCameras * 10),
      Math.ceil(totalCameras * 15)
    );
  }
